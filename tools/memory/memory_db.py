...
def _ensure_memory_entries_schema(conn: sqlite3.Connection, cursor: sqlite3.Cursor) -> None:
    """
    Ensure existing memory_entries table is compatible with this tool.

    Supports migration from legacy schema:
    id, content, entry_type, importance, created_at
    """
    if not _table_exists(cursor, "memory_entries"):
        return

    columns = _column_names(cursor, "memory_entries")

    if "type" not in columns and "entry_type" in columns:
        cursor.execute("ALTER TABLE memory_entries ADD COLUMN type TEXT")
        cursor.execute(
            "UPDATE memory_entries SET type = COALESCE(entry_type, 'fact') WHERE type IS NULL"
        )
    ...
    for column, definition in missing_columns:
        if column not in columns:
            cursor.execute(f"ALTER TABLE memory_entries ADD COLUMN {column} {definition}")

    # Backfill defaults and content hashes after migration.
    cursor.execute("UPDATE memory_entries SET type = 'fact' WHERE type IS NULL OR type = ''")
    cursor.execute("UPDATE memory_entries SET source = 'session' WHERE source IS NULL OR source = ''")
    cursor.execute("UPDATE memory_entries SET confidence = 1.0 WHERE confidence IS NULL")
    cursor.execute("UPDATE memory_entries SET access_count = 0 WHERE access_count IS NULL")
    cursor.execute("UPDATE memory_entries SET is_active = 1 WHERE is_active IS NULL")

    cursor.execute(
        "SELECT id, content FROM memory_entries WHERE content_hash IS NULL OR content_hash = ''"
    )
...
def init_db() -> None:
    """Initialize/upgrade schema once per process."""
    ...
    with _schema_lock:
        ...
        try:
            ...
            ''')
            _ensure_memory_entries_schema(conn, cursor)
...
def get_connection() -> sqlite3.Connection:
    """Get database connection (schema init is handled once by init_db())."""
    init_db()
...
def _fetch_entry(cursor: sqlite3.Cursor, entry_id: int) -> Dict[str, Any]:
    cursor.execute('SELECT * FROM memory_entries WHERE id = ?', (entry_id,))
    entry = row_to_dict(cursor.fetchone())
    if not entry:
...
def fetch_entry_readonly(entry_id: int) -> Dict[str, Any]:
    """Get a single memory entry by ID without updating access analytics."""
    conn = get_connection()
    try:
        cursor = conn.cursor()
        return _fetch_entry(cursor, entry_id)
    finally:
        conn.close()
...
def get_entry(entry_id: int) -> Dict[str, Any]:
    ...
    try:
        cursor = conn.cursor()
        result = _fetch_entry(cursor, entry_id)
...
def search_entries(
    ...
    # Escape wildcard chars so query is treated as literal text.
